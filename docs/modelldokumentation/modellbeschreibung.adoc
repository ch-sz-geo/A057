...
Auf dem verfügten Mutationsplan sind beide Linien, die Stockgrenze und die Waldgrenze, eingezeichnet. Als weiteren Typ kennt das kantonale Modell die dynamische Waldgrenzen. Diese wird jedoch vor der Publikation ins WebGIS und auf geodienste.ch herausgefiltert. Im WebGIS und auf geodienste.ch werden nur die statischen Waldgrenzen (in- und ausserhalb von Bauzonen) dargestellt. Diese sind ÖREB-relevant.

Die für den ÖREB-Kataster relevante Linie ist diejenige der statischen Waldgrenze.
